          ...
          {page === 'digest' && (
            <>
              <div style={{ marginBottom: 28 }}>
                ...
                <div style={{ fontSize: 13, color: '#555' }}>Get a summarized briefing of all your topics delivered to your inbox</div>
              </div>
              ...
              {topics.length === 0 ? (
                <div style={s.card}>
                  <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                    ...
                    <div style={{ fontSize: 13, color: '#3a3a3a' }}>Add topics in the sidebar first, then send a digest</div>
                  </div>
                </div>
              ) : (
                <>
                  <div style={s.card}>
                    <div style={{ fontSize: 14, fontWeight: 500, color: '#fff', marginBottom: 4 }}>Topics included in digest</div>
                    <div style={{ fontSize: 13, color: '#555', marginBottom: 20 }}>The following {topics.length} topic{topics.length > 1 ? 's' : ''} will be researched and summarized</div>
                    {topics.map(t => (
                  ...
                  <div style={s.card}>
                    <div style={{ fontSize: 14, fontWeight: 500, color: '#fff', marginBottom: 16 }}>Send digest to email</div>
                    <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                      <input
                        ...
                        style={{ flex: 1, fontSize: 13, padding: '9px 12px', borderRadius: 7, border: '0.5px solid #2a2a2a', background: '#1a1a1a', color: '#fff', outline: 'none' }}
                      />
                      <button onClick={sendDigest} disabled={digestLoading || !email.trim()} style={{ fontSize: 13, fontWeight: 500, padding: '9px 20px', borderRadius: 7, background: '#D4537E', color: '#fff', border: 'none', cursor: digestLoading ? 'not-allowed' : 'pointer', opacity: digestLoading ? 0.5 : 1, whiteSpace: 'nowrap' }}>
                    ...
                    <div style={{ fontSize: 12, color: '#444', marginTop: 12 }}>
                      Claude and Gemini will research each topic live and compile a fresh briefing before sending.
                    </div>
                  </div>
                </>
              )}
              ...
              <div style={s.card}>
                {loading ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '40px 0', fontSize: 14, color: '#555' }}>
                    ...
                    <span>Claude and Gemini are researching <strong style={{ color: '#aaa' }}>"{topic.name}"</strong> — takes 20–40 seconds...</span>
                  </div>
                ) : topic.report ? (
                  <>
                    ...
                    <div style={{ marginTop: 16, paddingTop: 14, borderTop: '0.5px solid #1a1a1a', display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center' }}>
                      ...
                      {['Claude', 'Gemini', 'Web search'].map(s => <span key={s} style={{ fontSize: 11, background: '#161616', color: '#444', border: '0.5px solid #222', borderRadius: 5, padding: '3px 8px' }}>{s}</span>)}
                    </div>
                  </>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 0', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 13, color: '#555', marginBottom: 22 }}>
                      Claude and Gemini will research this together and synthesize the best answer
                    </div>
      ...
      {showEmailModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: '#111', border: '0.5px solid #2a2a2a', borderRadius: 12, padding: 28, width: '100%', maxWidth: 400 }}>
            <div style={{ fontSize: 16, fontWeight: 500, marginBottom: 6 }}>Send daily digest</div>
            <div style={{ fontSize: 13, color: '#555', marginBottom: 20 }}>Claude and Gemini will research all {topics.length} topic{topics.length !== 1 ? 's' : ''} and email you a summary</div>
