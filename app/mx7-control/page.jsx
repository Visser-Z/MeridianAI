...
export default function AdminPage() {
  ...
  async function handleLogin() {
    ...
    const res = await fetch('/api/admin/user', {
      ...
      body: JSON.stringify({ email: 'test', action: 'activate', password: pw }),
    });
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch('/api/admin/user', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email.trim().toLowerCase(), action, password: pw }),
      });
      const data = await res.json();
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: '-apple-system, sans-serif' }}>
        <div style={{ background: '#111', border: '0.5px solid #1e1e1e', borderRadius: 12, padding: 40, width: '100%', maxWidth: 380 }}>
          ...
          <input type="password" value={pw} onChange={e => setPw(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} placeholder="••••••••" style={{ width: '100%', fontSize: 13, padding: '10px 12px', borderRadius: 7, border: '0.5px solid #2a2a2a', background: '#1a1a1a', color: '#fff', outline: 'none', marginBottom: 12, boxSizing: 'border-box' }} />
          {pwError && <div style={{ fontSize: 13, color: '#E24B4A', marginBottom: 12 }}>{pwError}</div>}
          <button onClick={handleLogin} style={{ width: '100%', padding: '11px', background: '#D4537E', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Enter</button>
        </div>
      </div>
    );
  }
      ...
      <div style={{ maxWidth: 600, margin: '0 auto' }}>
        ...
        <div style={{ background: '#111', border: '0.5px solid #1e1e1e', borderRadius: 12, padding: 32, marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            <button onClick={() => setAction('activate')} style={{ flex: 1, padding: '8px', borderRadius: 7, border: '0.5px solid', borderColor: action === 'activate' ? '#1D9E75' : '#2a2a2a', background: action === 'activate' ? '#0d2018' : '#1a1a1a', color: action === 'activate' ? '#1D9E75' : '#555', fontSize: 13, cursor: 'pointer' }}>Activate</button>
            <button onClick={() => setAction('deactivate')} style={{ flex: 1, padding: '8px', borderRadius: 7, border: '0.5px solid', borderColor: action === 'deactivate' ? '#E24B4A' : '#2a2a2a', background: action === 'deactivate' ? '#25100f' : '#1a1a1a', color: action === 'deactivate' ? '#E24B4A' : '#555', fontSize: 13, cursor: 'pointer' }}>Deactivate</button>
          </div>
          <div style={{ fontSize: 12, color: '#555', marginBottom: 6 }}>Client email address</div>
          <input value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} placeholder="client@company.com" style={{ width: '100%', fontSize: 13, padding: '10px 12px', borderRadius: 7, border: '0.5px solid #2a2a2a', background: '#1a1a1a', color: '#fff', outline: 'none', marginBottom: 16, boxSizing: 'border-box' }} />
          <button onClick={handleSubmit} disabled={loading || !email.trim()} style={{ width: '100%', padding: '11px', background: action === 'activate' ? '#1D9E75' : '#E24B4A', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: loading || !email.trim() ? 'not-allowed' : 'pointer', opacity: loading || !email.trim() ? 0.6 : 1 }}>
        ...
        <div style={{ background: '#111', border: '0.5px solid #1e1e1e', borderRadius: 12, padding: 32 }}>
          ...
          <div style={{ fontSize: 13, color: '#555', lineHeight: 1.8 }}>
            1. Client agrees to your monthly fee and pays via EFT<br/>
            2. Enter their email above and click Activate<br/>
            3. They visit the app, enter their email — first login prompts password setup<br/>
