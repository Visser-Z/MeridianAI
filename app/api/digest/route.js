      ...
      topics.map(async (topic) => {
        const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 300,
            messages: [
              { role: "system", content: "You are MeridianAI. Give a 3-sentence summary of the latest developments for the given topic. Be specific with numbers and data. Plain text only, no HTML or markdown." },
              { role: "user", content: `Give me a brief current summary of: "${topic.name}"` },
            ],
          }),
        });
        const data = await res.json();
        const summary = data.choices?.[0]?.message?.content || "Summary unavailable.";
        return { name: topic.name, mode: topic.mode, summary };
      })
    );
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
          ...
          <tr>
            <td style="background:#111111;border:1px solid #1e1e1e;border-top:none;border-radius:0 0 12px 12px;padding:20px 24px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="font-size:12px;color:#444444;">Delivered by MeridianAI · Next digest tomorrow at 7:00 AM</td>
                  <td align="right"><span style="display:inline-block;font-size:11px;font-weight:600;background:#2a0f1a;color:#D4537E;padding:4px 10px;border-radius:4px;">Pro plan</span></td>
                </tr>
              </table>
            </td>
          </tr>
